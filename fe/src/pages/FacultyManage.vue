...
function normalizeStatus(value) {
  return STATUS_OPTIONS.includes(value) ? value : 'Hoạt động'
}

function statusClass(status) {
  if (status === 'Tạm nghỉ') return 'paused'
  if (status === 'Dừng hoạt động') return 'stopped'
  return ''
}
...
function resetDetail() {
  ...
  detail.name = ''
  detail.description = ''
  detail.head_teacher_code = ''
  detail.head_teacher_name = ''
  detail.status = ''
  detail.class_count = 0
...
async function openDetail(row) {
  ...
  detail.head_teacher_code = String(row?.head_teacher_code || '')
  detail.head_teacher_name = String(row?.head_teacher_name || '')
  detail.status = String(row?.status || '')
  detail.class_count = Number(row?.class_count || 0)
  detail.student_count = Number(row?.student_count || 0)
  detailVisible.value = true

  const code = String(row?.code || '').trim()
  if (!code) {
    detailError.value = 'Thiếu mã khoa để xem chi tiết.'
    return
  }
  ...
  try {
    ...
    if (!res.ok || payload.status !== 'success' || !payload.data) {
      detailError.value = payload.message || 'Không thể tải chi tiết khoa.'
      return
    }
    ...
    detail.head_teacher_code = String(item.head_teacher_code || '')
    detail.head_teacher_name = String(item.head_teacher_name || '')
    detail.status = normalizeStatus(String(item.status || ''))
    detail.class_count = Number(item.class_count || 0)
    detail.student_count = Number(item.student_count || 0)
  } catch (error) {
...
function goCreate() {
  router.push({ name: 'faculty-form', query: { mode: 'create' } })
}

function goEdit(row) {
  router.push({ name: 'faculty-form', query: { mode: 'edit', code: row.code } })
}
...
async function loadData() {
  ...
  try {
    ...
    summary.total_classes = Number(payload.summary?.total_classes || 0)
    summary.total_students = Number(payload.summary?.total_students || 0)
    searched.value = true
  } catch (error) {
...
async function deleteFaculty(row) {
  const code = String(row?.code || '').trim()
  if (!code) return
  if (!window.confirm(`Bạn có chắc muốn xóa khoa ${code}?`)) return

  deletingCode.value = code
  errorMessage.value = ''
  successMessage.value = ''
  try {
    const res = await fetch(`/api/faculties?code=${encodeURIComponent(code)}`, { method: 'DELETE' })
    const payload = await res.json().catch(() => ({}))
    if (!res.ok || payload.status !== 'success') {
      errorMessage.value = payload.message || 'Không thể xóa khoa.'
      return
    }
    successMessage.value = 'Đã xóa khoa.'
    await loadData()
  } catch (error) {
    errorMessage.value = 'Không kết nối được máy chủ.'
  } finally {
  ...
  <div class="page">
    <div class="card">
      <div class="header-row">
        <div>
          <h1>Quản lý khoa</h1>
          <p class="subtitle">Quản lý danh sách các khoa trong trường.</p>
        </div>
        <button class="btn-add" type="button" @click="goCreate">+ Thêm khoa mới</button>
      </div>
      ...
      <div class="toolbar">
        <input
          ...
          type="text"
          placeholder="Tìm kiếm theo mã khoa, tên khoa, mã trưởng khoa..."
      ...
      <div class="table-wrap">
        ...
        <div v-else-if="searched && rows.length === 0" class="state">Không có khoa phù hợp.</div>
            ...
            <tbody>
              <tr v-for="row in rows" :key="row.code">
                <td>
                  ...
                  <div class="dept-code">Mã: {{ row.code }}</div>
                </td>
                <td>
                  <div>{{ row.head_teacher_name || '-' }}</div>
                  <div class="dept-code">{{ row.head_teacher_code || '-' }}</div>
                </td>
                <td>{{ row.class_count || 0 }}</td>
    ...
    <div v-if="detailVisible" class="modal-backdrop" @click.self="closeDetail">
      <div class="modal-card">
        ...
        <div v-else class="detail-grid">
          <div><b>Mã khoa:</b> {{ detail.code || '-' }}</div>
          <div><b>Tên khoa:</b> {{ detail.name || '-' }}</div>
          <div><b>Trưởng khoa:</b> {{ detail.head_teacher_name || '-' }}</div>
          <div><b>MSGV trưởng khoa:</b> {{ detail.head_teacher_code || '-' }}</div>
          <div><b>Trạng thái:</b> {{ detail.status || '-' }}</div>
          <div><b>Số lớp:</b> {{ detail.class_count }}</div>

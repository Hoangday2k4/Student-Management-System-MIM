...
onMounted(async () => {
  try {
    ...
    if (!p) {
      serverError.value = payload.message || 'Không có dữ liệu hồ sơ sinh viên.'
      return
    }
    form.student_code = p.student_code || ''
...
<template>
  <div class="page">
    <div class="card">
      <h1>Cập nhật hồ sơ sinh viên</h1>
      <p v-if="loading">Đang tải...</p>

      <form v-else-if="step === 'input'" class="grid" @submit.prevent="goConfirm">
        <label>MSSV</label>
        <input :value="form.student_code" type="text" disabled />
      ...
      <div v-else class="confirm-box">
        ...
        <div class="grid">
          <span class="label">MSSV</span><span>{{ form.student_code }}</span>

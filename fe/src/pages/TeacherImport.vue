...
async function doPreview() {
  ...
  try {
    ...
    const res = await fetch('/api/teacher_import.php?action=preview', {
      method: 'POST',
      body: formData,
    })
...
async function doSave() {
  ...
  errorMessage.value = ''
  try {
    const res = await fetch('/api/teacher_import.php', {
      method: 'POST',
...
<template>
  <div class="page">
    <div class="card">
      <h1>Nhập danh sách giáo viên</h1>

      <div v-if="saveResult" class="success-box">
        <p>Nhập thành công: <strong>{{ saveResult.inserted_count }}</strong> giáo viên.</p>
        <p v-if="saveResult.skipped_count > 0">Bỏ qua: {{ saveResult.skipped_count }} dòng.</p>
        <button class="btn-ghost" @click="router.push('/teachers/search')">Về danh sách</button>
      </div>
      ...
      <template v-else>
        ...
        <div v-if="previewDone">
          ...
          <table v-else class="result-table">
            ...
            <tbody>
              <tr v-for="row in previewRows" :key="row.teacher_code">
          ...
          <div class="actions" style="margin-top: 14px;">
            <button class="btn-primary" :disabled="saving || previewRows.value?.length === 0" @click="doSave">

...
onMounted(async () => {
  try {
    ...
    if (!p) {
      error.value = payload.message || 'Khong co du lieu ho so giao vien.'
      return
    }
    profile.value = p
  } catch (e) {
  ...
  <div class="page">
    <div class="card">
      ...
      <div v-else-if="profile" class="grid">
        ...
        <div class="info">
          <p><b>Mã GV:</b> {{ profile.teacher_code }}</p>
      ...
      <div class="actions">
        <button class="btn-primary" @click="router.push('/teachers/profile/update')">Cập nhật hồ sơ</button>
